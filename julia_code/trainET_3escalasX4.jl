using DSP, Wavelets, JLD, Combinatorics, ExtremelyRandomizedTrees
...
println("\nEntrenando ExtraTrees Classifier")

@time pmodel = ExtraTrees(norm_instances', all_labels'+1, ntrees=nTrees, k=nselFeat)

@time aux_plabels = predict(pmodel, norm_instances')
    predicted_labels=vec(round(aux_plabels-1))
...
out=open("../model/ETmodelX4_Ch1_$(dim)_randFeats$(nselFeat)_nTrees$(nTrees)_dwt_levels$(dwt_levels).jls", "w")
